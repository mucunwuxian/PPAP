...
# 
def impadding_mirror(img, padding_vert, padding_horz=None, random_range=None, decay=None):
    # 
    if (decay is None):
        # 
        decay = 0.0
    ...
    if (random_range is None):
        for padding_i in range(np.max([padding_vert, padding_horz])):
            ...
            if (padding_i < padding_horz):
                # 
                img_padding_left   = (img[:, 0][:, np.newaxis]  + ((255 - img[:, 0][:, np.newaxis])  * decay))
    ...
    # 
        ...
        # 
        padding_top    = int(np.round(padding_vert * (random_range[0] + (np.random.rand(1) * (random_range[1] - random_range[0])))))
        padding_left   = int(np.round(padding_horz * (random_range[0] + (np.random.rand(1) * (random_range[1] - random_range[0])))))
        padding_bottom = int(np.round(padding_vert * (random_range[0] + (np.random.rand(1) * (random_range[1] - random_range[0])))))
        padding_right  = int(np.round(padding_horz * (random_range[0] + (np.random.rand(1) * (random_range[1] - random_range[0])))))
        # 
        for padding_top_i in range(padding_top):
            img = np.concatenate([img[:, 0][:, np.newaxis], img], axis=1) 
        for padding_left_i in range(padding_left):
            img = np.concatenate([img[0, :][np.newaxis, :], img], axis=0) 
        for padding_bottom_i in range(padding_bottom):
            img = np.concatenate([img, img[:, -1][:, np.newaxis]], axis=1)
        for padding_right_i in range(padding_right):
    ...
    for aug_i in range(len(img_aug)):
        ...
        if (color_adjust == True):
            ...
            if (roulette < 0.8):
                ...
                img_aug_tmp = img_aug_tmp / np.max(img_aug_tmp)
...
def split_image_from_grid(img_gray, 
    ...
    for line_coord_y_i in range(len(line_coord_y) - 1):
        ...
        for line_coord_x_i in range(len(line_coord_x) - 1):
            ...
            # 
            coord_top    = line_coord_y[crop_i + crop_j] + margin
            coord_left   = line_coord_x[crop_k] + margin
            coord_bottom = line_coord_y[crop_i + crop_j + 1] - margin
            coord_right  = line_coord_x[crop_k + 1]  - margin
            # 
            ...
            # 
            img_crop     = img_tmp[coord_top:coord_bottom, coord_left:coord_right]
        ...
        for line_coord_x_i in range(len(line_coord_x) - 1):
            ...
            for bins_i in range(len(bins_tmp) - 1):
                aspect_ratio_tmp_[bins_i] = float((aspect_ratio_tmp <= bins_tmp[bins_i]) & 
                                                  (bins_tmp[bins_i] <  aspect_ratio_tmp))
            # 
